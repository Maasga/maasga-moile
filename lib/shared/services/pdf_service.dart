...
class PdfService {
  ...
  static Future<Uint8List> _buildMaintenanceInvoicePdf(
      ...
      pw.MultiPage(
        ...
        build: (pw.Context context) {
          return [
                  ...
                  (v) => pw.TableRow(
                    children: [
                      _tableCell(
                        DateFormat(
                          'dd/MM/yyyy',
                        ).format(DateTime.parse(v['visit_date'])),
                        align: pw.TextAlign.left,
                      ),
                      _tableCell(
                        v['visit_type'] == 'preventive'
                            ? 'Préventive'
                            : v['visit_type'].toString(),
                        align: pw.TextAlign.left,
                      ),
                      _tableCell(
                  ...
                  child: pw.Column(
                    children: [
                      ...
                      _totalLine(
                        'Mode de paiement',
                        payment != null
                            ? (payMethodLabels[payment['method']] ??
                      ...
                      pw.Container(
                        ...
                        child: _totalLine(
                          'TOTAL PAYÉ',
                          '${_formatPrice((contract['plan_price'] ?? 0).toDouble())} FCFA',
                          color: primaryColor,
                        ),
                      ),
                    ],
            ...
            pw.Center(
              child: pw.Text(
                'Merci de votre confiance. MAASGA - Le froid en toute sérénité.',
                style: pw.TextStyle(
                  fontSize: 8,
  ...
  static Future<void> generateOrderDevis({
    ...
    if (commande.btu != null &&
        !displayName.contains(commande.btu.toString())) {
  ...
  static Future<void> generateOrderInvoice({
    ...
    items.add(
      _OrderPdfItem(
        name:
            ...
            (commande.btu != null ? ' (${commande.btu} BTU)' : ''),
        price: unitPrice,
        quantity: unitQty,
    ...
    final bool hasExtras =
        (commande.installationPrix ?? 0) > 0 ||
        (commande.accessoires != null && commande.accessoires!.isNotEmpty);
    ...
    if (hasExtras) {
      ...
      final remisePct = commande.remise ?? 0;
      final double devisTotalExtras =
          subTotalExtras - (subTotalExtras * remisePct / 100);

      if (devisTotalExtras > 0) {
        items.add(
          _OrderPdfItem(
            name:
                'Installations & Prestations (Réf. ${commande.devisNumero ?? 'Devis'})',
          ...
          children: [
            pw.Expanded(
              child: _buildInfoCard('INFORMATIONS CLIENT', [
                ...
                if (clientId != null)
                  'Réf. client : CLI-${clientId.toString().padLeft(5, '0')}',
              ]),
            ),
              ...
              (item) => pw.TableRow(
                children: [
                  _tableCell(
                    item.name,
                    align: pw.TextAlign.left,
                    isBold: item.name == items.first.name,
                  ),

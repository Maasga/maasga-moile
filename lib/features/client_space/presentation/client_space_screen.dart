...
class _LoggedInView extends ConsumerWidget {
  ...
  Future<void> _handleAcceptDevis(
    WidgetRef ref,
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          'Souhaitez-vous confirmer l\'acceptation de ce devis ? Notre équipe vous contactera sous 24h pour planifier l\'intervention.',
        ),
        actions: [
          TextButton(
  ...
  Future<void> _handleRejectDevis(
    WidgetRef ref,
    BuildContext context,
    String orderId,
  ) async {
    ...
    if (confirmed == true) {
      try {
        ...
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Devis refusé. Nous en prenons note.'),
            ),
          );
        }
      } catch (e) {
      ...
      child: Column(
        ...
        children: [
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _ProfileInfoRow(
                        ...
                        text: profile.quartier,
                        iconColor: const Color(0xFFFF6F00),
                      ),
                      const SizedBox(height: 4),
                      _ProfileInfoRow(
                        icon: Icons.calendar_today,
                        text: 'Membre depuis ${profile.memberSince}',
                        fontSize: 12,
                      ),
                    ],
                  ),
              ...
              child: Row(
                children: [
                  ...
                  StatItem(
                    ...
                    count:
                        '${data.orders.where((o) => o.status == CommandeStatus.installed).length}',
                    label: 'Installations',
                  ),
                  _VerticalDivider(),
                  StatItem(
                  ...
                  StatItem(
                    ...
                    count:
                        '${data.rdvs.where((r) => r.status == RdvStatus.enAttente || r.status == RdvStatus.confirme).length}',
                    label: 'RDV en cours',
                  ),
                  _VerticalDivider(),
                  StatItem(
                    icon: Icons.shield_outlined,
                    count: '${data.maintenanceContracts.length}',
                    label: 'Maintenance',
                  ),
                  const SizedBox(width: 16),
                ],
                    ...
                    children: data.orders
                        .map(
                          (o) => CommandeCard(
                            commande: o,
                            onViewInvoice: () =>
                                PdfService.generateOrderInvoice(
                                  profile: data.profile,
                                  commande: o,
                                ),
                            onDownloadDevis: () =>
                                PdfService.generateOrderDevis(
                                  profile: data.profile,
                                  commande: o,
                                ),
                            ...
                            onAcceptDevis: () =>
                                _handleAcceptDevis(ref, context, o.id),
                            onRejectDevis: () =>
                                _handleRejectDevis(ref, context, o.id),
                            onCancel: () =>
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                    content: Text(
                                      'Action "Annuler et rembourser" bientôt disponible',
                                    ),
                                  ),
                                ),
                          ),
          ...
          SectionCard(
            ...
            count: data.maintenanceContracts
                          ...
                          (c) => ContratCard(
                            ...
                            onViewInvoice: () async {
                              try {
                                ...
                                await PdfService.displayRemoteInvoice(
                                  dio: dio,
                                  contractId: c.id,
                                );
                              } catch (e) {
                                if (context.mounted) {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                      content: Text(
                                        'Erreur lors du téléchargement de la facture: $e',
                                      ),
                                    ),
                                  );
                                }
...
class _ActionBtn extends StatelessWidget {
  const _ActionBtn({
    ...
    this.icon,
    this.isFilled = false,
    this.color = const Color(0xFF1B3A8D),
  });
  final String label;
